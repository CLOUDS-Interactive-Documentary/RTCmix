	...
	*retval = 0.0;	// Default to float 0
	...
	if (instCreator) {
        ...
        if (rv != (double) DONT_SCHEDULE) { // only schedule if no setup() error
			...
			if (RTcmix::rtInteractive) {
			   if (Iptr->configure(RTcmix::RTBUFSAMPS) != 0) {
...
extern "C" {
	...
	Handle iwrap(const Arg args[], const int nargs);
	int killit(const Arg args[], const int nargs);
}
...
// BGGx -- kill thing for Luke
Handle
iwrap(const Arg arglist[], const int nargs)
{
	Instrument *Iptr = NULL;;
	const char *instName = (const char *) arglist[0];
...
int killit(const Arg arglist[], const int nargs)
{
